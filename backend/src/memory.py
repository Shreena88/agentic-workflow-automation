...
class FAISSMemoryStore:
    ...
    def clear(self, session_id: str) -> None:
        """Clear session memory from in-memory cache and delete its persisted directory."""
        if session_id in self._indexes:
            del self._indexes[session_id]
        if session_id in self._texts:
            del self._texts[session_id]
    ...
    def retrieve(self, session_id: str, query: str, top_k: int = 5) -> list[str]:
        ...
        for dist, idx in zip(distances[0], indices[0]):
            if idx >= 0 and dist < DISTANCE_THRESHOLD:
                # Truncate each memory snippet to 3000 chars to avoid prompt bloating
                txt = texts[idx]
                if len(txt) > 3000:
                    txt = txt[:3000] + "..."
                results.append(txt)
