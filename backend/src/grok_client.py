...
- summarize: summarize a piece of text using LLM. Use this tool to condense multiple search results or long documents into an understandable format.
- read_file: read a local file (PDF or text)
- analyze: run pandas data analysis on a CSV and excel file
...
- Only use 'read_file' if the user provides a filename or the 'Context' indicates a file has been uploaded. Do NOT guess filenames.
- Do NOT plan redundant summarize tasks if a previous search or file read failed.
...
class GrokClient:
    ...
    def chat(self, system: str, user: str, temperature: float = 0.2) -> str:
        """Send a chat completion request with built-in error handling."""
        try:
            ...
            if "rate_limit" in str(e).lower() or "429" in str(e):
                logger.error("Grok Rate Limit hit: %s", e)
    ...
    def generate_final_response(self, query: str, results: dict[str, str]) -> str:
        """Compose a final answer from tool results, with heavy truncation to prevent TPD Errors."""
